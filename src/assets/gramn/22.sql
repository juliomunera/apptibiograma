...
FROM
	(
		...
		WHERE
			g.tipoGRAM = '-' AND
			g.idBacteria IN (21,22,24,25,28)
	) g1,
	(
		SELECT
		...
		FROM
			...
			Asignaciones a
		WHERE
			(dp1.idParteDelCuerpo = 0 AND a.id IN (36,40,39)) OR
			(dp1.idParteDelCuerpo = 1 AND a.id IN (36,37,16)) OR
			(dp1.idParteDelCuerpo = 2 AND a.id IN (36,40,39)) OR
			(dp1.idParteDelCuerpo = 3 AND a.id IN (47,37,40,39,16)) OR
			(dp1.idParteDelCuerpo = 4 AND a.id IN (37,36,18,21)) OR
			(dp1.idParteDelCuerpo = 5 AND a.id IN (36,40)) OR
			(dp1.idParteDelCuerpo = 6 AND a.id IN (36,22,40,39)) OR
			(dp1.idParteDelCuerpo = 7 AND a.id IN (36,40,39,16)) OR
